...
function handler (req, res) { // handler za "response"; ta handler "handla" le datoteko index.html
    ...
    case ('/') : // in the case of default page

    fs.readFile(__dirname + "/Untitled19.html",
    function (err, data) { // callback function for txt file read
        if (err) {
            res.writeHead(500);
    ...
    fs.readFile(__dirname + "/model1.html",
    function (err, data) { // Callback function for reading txt file
        if (err) {
            res.writeHead(500);
            return res.end("Error loading page admin...html");
        }
    ...
    fs.readFile(__dirname + "/model2.html",
    function (err, data) { // Callback function for reading txt file
        if (err) {
            res.writeHead(500);
            return res.end("Error loading page admin...html");
...
var pwmLimit = 110; // to limit value of the pwm that is sent to the motor
